...
module.exports = function injectBinary(options, reqSelectors, resSelectors, htmlOnly) {
  ...
  function prepareResponseSelectors(req, res) {
    ...
    res.writeHead = function () {
      ...
      if ( res.isHtml() && !res.isRedirect() && !res.isGzipped()) {
        ...
        var zitiConfig = common.generateZitiConfigObject( '', req, options);
        zitiConfig = JSON.stringify(zitiConfig);
        zitiConfig = Base64.encode(zitiConfig);
        let domain;
        if (env('ZITI_BROWZER_BOOTSTRAPPER_WILDCARD_VHOSTS')) {
          domain = `${req.ziti_vhost}.${ common.trimFirstSection( env('ZITI_BROWZER_BOOTSTRAPPER_HOST') )}`
        } else {
          domain = `${req.ziti_vhost}`;
        }      
        cookies.push(
          cookie.serialize(
            '__ziti-browzer-config', zitiConfig, 
            {
              sameSite: true,
              path:     '/',
              domain:   `${domain}`,
