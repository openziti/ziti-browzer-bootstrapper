...
module.exports = {
  ...
  stream: async function stream(req, res, options, _, server, clb) {
    ...
    if ((rtRequest > 0)) {
      ...
      let pathToZitiBrowzerRuntimePkg = pathToZitiBrowzerRuntimeModule.substring(0, pathToZitiBrowzerRuntimeModule.lastIndexOf('/'));
      pathToZitiBrowzerRuntimePkg = pathToZitiBrowzerRuntimePkg.substring(0, pathToZitiBrowzerRuntimePkg.lastIndexOf('/'));
      var zbrPkg = JSON.parse(fs.readFileSync(pathToZitiBrowzerRuntimePkg+'/package.json', 'utf8'));
      let zbrVersion = zbrPkg.version;

      // Read the component off the disk
      let rtFileName = outgoing.path.split("/").pop();
      rtFileName = rtFileName.split("?")[0];
      fs.readFile( path.join( pathToZitiBrowzerRuntimeModule, rtFileName ), (err, data) => {
        ...
        if (err) {  // If we can't read the file from disk
          ...
          res.writeHead(200, { 
            ...
            'x-ziti-http-agent-info': `ziti browzer runtime v${zbrVersion}` 
          });
      ...
      fs.readFile( path.join( pathToZitiBrowzerRuntimeModule, rtwasmFileName ), (err, data) => {
        ...
        if (err) {  // If we can't read the file from disk
          ...
          res.writeHead(200, { 
            ...
            'x-ziti-http-agent-info': 'ziti browZer WebAssembly' 
          });
    ...
    if ((swRequest > 0)) {
      ...
      res.writeHead(200, { 
        ...
        'x-ziti-http-agent-info': 'ziti browzer service worker' 
      });
    ...
    res.writeHead(
      ...
      'x-ziti-http-agent-info': 'load ziti browzer Runtime' 
    });
