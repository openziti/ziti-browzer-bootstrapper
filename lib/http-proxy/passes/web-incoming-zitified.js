...
module.exports = {
  ...
  stream: async function stream(req, res, options, _, server, clb) {
    ...
    if (!net.isIP(req.headers['host'])) {
      options.logger.debug({message: 'req start', clientIp: requestIp.getClientIp(req), method: req.method, url: req.url});
    }

    //
    if (typeof req.ziti_target_service === 'undefined') {
      res.writeHead(404, { 'X-Ziti-BrowZer-Gateway': `vhost [${req.headers['host']}] unknown` });
      res.end('');
      if (!net.isIP(req.headers['host'])) {
        options.logger.warn({message: 'req terminate; vhost unknown', vhost: req.headers['host']});
      }
      return;
    ...
    if ((rtNakedRequest > 0)) {
      options.logger.debug({message: 'Request for naked ziti-browzer-runtime.js', component: 'JS', clientIp: requestIp.getClientIp(req), method: req.method, url: req.url});
      ...
      options.logger.debug({message: 'ziti-browzer-runtime SHA filename: ' + rtFileName, clientIp: requestIp.getClientIp(req), method: req.method, url: req.url});

      res.writeHead(302, {
        'Location': `${rtFileName}`,
        'X-Ziti-BrowZer-Gateway': 'ziti-browzer-runtime SHA filename' 
      });
      res.end('');
    ...
    if ((corsProxyRequest > 0)) {
      options.logger.debug({message: 'beginning CORS Proxy', clientIp: requestIp.getClientIp(req), method: req.method, url: req.url});
    ...
    if (req.method !== 'GET') {
      res.writeHead(403, { 'X-Ziti-BrowZer-Gateway': 'non-GET methods are prohibited' });
      res.end('');
      options.logger.debug({message: 'req terminate; non-GET method', clientIp: requestIp.getClientIp(req), method: req.method, url: req.url});
      return;
    }
    ...
    if ((rtRequest > 0)) {
      options.logger.debug({message: 'Request for ziti-browzer-runtime', component: 'JS', clientIp: requestIp.getClientIp(req), method: req.method, url: req.url});
      ...
      fs.readFile( path.join( pathToZitiBrowzerRuntimeModule, rtFileName ), (err, data) => {
        ...
        if (err) {  // If we can't read the file from disk
          ...
          res.writeHead(500, { 'X-Ziti-BrowZer-Gateway': err.message });
          res.end('');
          return;
          ...
          res.writeHead(200, { 
            ...
            // 'Content-Security-Policy': "script-src 'self' 'unsafe-inline' 'unsafe-eval' blob:; worker-src 'self' 'unsafe-inline' 'unsafe-eval' blob:;"
    ...
    if ((cssRequest > 0)) {
      options.logger.debug({message: 'Request for ziti-browzer-css.css', component: 'CSS', clientIp: requestIp.getClientIp(req), method: req.method, url: req.url});
    ...
    if ((logoRequest > 0)) {
      options.logger.debug({message: 'Request for ziti-browzer-logo.svg', component: 'SVG', clientIp: requestIp.getClientIp(req), method: req.method, url: req.url});
    ...
    if ((rtwasmRequest > 0)) {
      options.logger.debug({message: 'Request for ziti-browzer-runtime libcrypto.wasm', component: 'WASM', clientIp: requestIp.getClientIp(req), method: req.method, url: req.url});
      ...
      fs.readFile( path.join( pathToZitiBrowzerRuntimeModule, rtwasmFileName ), (err, data) => {
        ...
        if (err) {  // If we can't read the file from disk
          ...
          res.writeHead(200, { 
            ...
            'X-Ziti-BrowZer-Gateway': 'OpenZiti browZer WebAssembly' 
          });
    ...
    if ((hystmodalRequest > 0)) {
      options.logger.debug({message: 'Request for ziti-browzer-UI', component: 'UI', clientIp: requestIp.getClientIp(req), method: req.method, url: req.url});
      ...
      fs.readFile( path.join( pathToHystmodalModule, 'dist', hystmodalFileName ), (err, data) => {
        ...
        if (err) {  // If we can't read the file from disk
          ...
          res.writeHead(200, { 
            ...
            'X-Ziti-BrowZer-Gateway': 'OpenZiti browZer Hystmodal' 
          });
    ...
    if ((polipopRequest > 0)) {
      ...
      fs.readFile( path.join( pathToZitiBrowzerRuntimeModule, polipopFileName ), (err, data) => {
        ...
        if (err) {  // If we can't read the file from disk
          ...
          res.writeHead(200, { 
            'Content-Type': `${contentType}`,
            'X-Ziti-BrowZer-Gateway': 'OpenZiti browZer Polipop' 
          });
    ...
    if ((hotkeysRequest > 0)) {
      ...
      fs.readFile( path.join( pathToHotkeysModule, 'dist', hotkeysFileName ), (err, data) => {
        ...
        if (err) {  // If we can't read the file from disk
          ...
          res.writeHead(200, { 
            'Content-Type': `${contentType}`,
            'X-Ziti-BrowZer-Gateway': 'OpenZiti browZer Hotkeys' 
          });
    ...
    if ((swRequest > 0)) {
      options.logger.debug({message: 'Request for ziti-browzer-sw', component: 'JS', clientIp: requestIp.getClientIp(req), method: req.method, url: req.url});
      ...
      res.writeHead(200, { 
        ...
        'X-Ziti-BrowZer-Gateway': 'OpenZiti browZer Service Worker' 
      });
    ...
    let requestIsTargetPath = common.isRequestOnTargetPath( req, options, outgoing.path );
    if ( requestIsTargetPath ) {
      options.logger.debug({message: 'Request for (targetPath) root, we will load page', clientIp: requestIp.getClientIp(req), method: req.method, url: req.url});
    }

    if (!requestIsTargetPath) {
      options.logger.warn({message: 'req terminate; request is NOT on target-path', clientIp: requestIp.getClientIp(req), method: req.method, url: req.url});
      res.writeHead(302, {
        'Location': `${options.targetPath}`,
        'X-Ziti-BrowZer-Gateway': 'request is NOT on target-path' 
      });
      res.end('');
      return;
    }
    ...
    res.writeHead(
      ...
      'X-Ziti-BrowZer-Gateway': 'load OpenZiti browZer Runtime' 
    });
