    ...
    return new Promise( async (resolve) => {
        ...
        if (
            ...
            // Ensure the browser, not this service worker, does the (re)fetch of the page that caused sdk injection.
                ...
                // console.log('ziti-sw: shouldDoInterceptInJSSDK now doing getCookies for: ', url);
                // resp = await sendMessageToClient( client, { command: 'getCookies', options: { logLevel: zitiConfig.httpAgent.zitiSDKjs.logLevel } } );
                // console.log('ziti-sw: shouldDoInterceptInJSSDK getCookies resp is: : ', resp);
    ...
    event.respondWith(async function() {
        ...
        return new Promise( async (resolve) => {
            ...
            if ( -1 == doInterceptInJSSDK ) {
                ...
                neweaders.append( 'referer', event.request.referrer );
...
/**
 *  setCookie 'message' handler
 */
 _onMessage_setCookie = async ( event ) => {
    let name = event.data.name;
    let value = event.data.value;
    if (typeof self.cookieObject !== 'undefined') {
 ...
 */
 addEventListener('message', event => {
    ...
    else if (event.data.command === 'setCookie')            { _onMessage_setCookie( event ); }
