...
async function sendMessageToClient( client, message ) {
    ...
    return new Promise( async function(resolve, reject) {
        ...
        messageChannel.port1.onmessage = function( event ) {
            console.log('ziti-sw: sendMessageToClient() onmessage(): reply event is: : ', event);
            if (event.data.error) {
...
async function shouldIntercept( event ) {
    ...
    return new Promise( async (resolve) => {
        ...
        console.log('ziti-sw: shouldIntercept entered for: ', method, url);
        ...
        if (url.match( regex )) {   // request is indeed targeting Ziti HTTP Agent
            ...
            if ( requestUrl.pathname === '/' ) {
                ...
                console.log('ziti-sw: shouldIntercept returning FALSE for: ', url);
                resolve( false );
                return;
            }
            ...
            // Ensure the browser, not this service worker, does the (re)fetch of the page that caused sdk injection.
                ...
                console.log('ziti-sw: shouldIntercept TRUE for: ', url);
...
// }
                ...
                console.log('ziti-sw: await clients.get for clientId: ', clientId);
                const client = await self.clients.get(clientId);
      
                console.log('ziti-sw: shouldIntercept now doing awaitIdentityLoaded for: ', url);
                ...
                console.log('ziti-sw: shouldIntercept returning TRUE for: ', url);
                resolve( true );
                return;
...
self.addEventListener('fetch', async function( event ) {
    ...
    event.respondWith(async function() {
        ...
        return new Promise( async (resolve) => {
            ...
            console.log( 'ziti-sw: inside event.respondWith, BEFORE shouldIntercept() for: ', event.request.url);
            var doIntercept = await shouldIntercept( event ).catch( async (err) => {

                console.log( 'ziti-sw: shouldIntercept error: fetch NOT intercepting', err); 
            ...
            console.log( 'ziti-sw: inside event.respondWith, AFTER  shouldIntercept() for: ', event.request.url, doIntercept);

            if ( !doIntercept ) {

                console.log( 'ziti-sw: fetch NOT intercepting: ', event.request.url); 
