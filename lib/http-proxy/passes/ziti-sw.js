    ...
    return new Promise( async (resolve) => {
        ...
        if (
            ...
            // Ensure the browser, not this service worker, does the (re)fetch of the page that caused sdk injection.
                ...
                /**
                 * We can't enable any intercepts until we have a Cert
                 */

                /**
                 * If there is no evidence that an identity has been loaded, and we have checked, and failed,
                 * at least once in the last second... then skip this intercept because
                 */
                ...
                console.log('ziti-sw: await sendMessageToClient: isIdentityPresent: ', url);
                let resp = await sendMessageToClient( client, { command: 'isIdentityPresent' } );
                console.log('ziti-sw: sendMessageToClient returned: ', url, resp);
                if (resp === '0') {
                    ...
                    // We'll skip intercept of this request (it will then hit the HTTP Agent and fail there). We expect the
