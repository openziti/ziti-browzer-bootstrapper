...
common.generateZitiConfigObject = function(url, req, options) {
  ...
  var u = new URL(`https://${agent_host}`);

  var target = find(options.targetArray, {
    vhost: u.hostname
  });
  if (typeof target === 'undefined') {
    options.logger.error({message: 'Host header has no match in targets array', host: agent_host});
...
common.isRequestOnTargetPath = function( req, options, path ) {
  ...
  if (req.ziti_target_path) {
    ...
    if ((hit > 0)) {
      options.logger.debug({message: 'common.isRequestOnTargetPath: HIT on path', path: path, clientIp: requestIp.getClientIp(req), method: req.method, url: req.url});
      result = true;
    }
  }
